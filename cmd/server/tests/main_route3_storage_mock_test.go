...
func TestServerHappyPathMockStorageV3(t *testing.T) {
	...
	modelTest10 := &models.Metric[uint64]{Type: "counter", Name: "test10_counter", Value: 10}
	...
	mockMetricStorage.EXPECT().
		...
		Return(modelTest10, nil).AnyTimes()
	...
	tests := []struct {
	...
	}{
		{
			...
			want: Want{
				...
				response:    `{"id":"test10_counter","type":"counter","delta":20}`,
			},
		},
	}
