...
./metricstest -test.v -test.run=^TestIteration14 -agent-binary-path=cmd/agent/agent -binary-path=./cmd/server/server -source-path=. -server-port=8080 -file-storage-path ./test.json -database-dsn postgresql://app:app123@127.0.0.1:5432/metric_db?sslmode=disable -key 1
